...
unlabeled_train_loader = data.DataLoader(unlabeled_dataset, batch_size=64)
#####################################################################################################################################################
def train():
    ...
    model_SAR = models.resnet50(pretrained=True)
    ...
    model_SAR = torch.load('EO_focal_loss_pretrained_resnet152.pth')
    ...
    for epoch in range(100):
        ...
        for i, (data1,data2) in enumerate(tqdm(train_loader)):
            ...
            inputs_EO, labels_EO, inputs_SAR, labels_SAR = inputs_EO.to(device_1), labels_EO.to(device_1), inputs_SAR.to(device_2), labels_SAR.to(device_1)
            ...
            outputs_SAR_unlabeled = model_SAR(SAR_inp_unlabeled).to(device_1)
    ...
    print()
    torch.save(model_EO, 'EO_cross_domain_resnet50.pth')
    torch.save(model_SAR, 'SAR_cross_domain_resnet50.pth')
    print()
